...
class Shinigami : Madara("Shinigami", "https://shinigami09.com", "id"), ConfigurableSource {
    ...
    // Mengubah URL gambar menggunakan layanan images.weserv.nl
    return data.mapIndexed { idx, it ->
        val resizedImageUrl = resizeImageUrl(it) // Resizing gambar menggunakan layanan
        Page(idx, document.location(), resizedImageUrl) // Menggunakan URL yang sudah diresize
    }
}

private fun resizeImageUrl(originalUrl: String): String {
    // URL gambar yang sudah diproses untuk meresize dengan layanan Weserv
